 ...
 * - Carga inicial desde localStorage o desde productosBase
 * - Escucha actualizaciones desde el panel admin
...
export function useProductos() {
  ...
  useEffect(() => {
    ...
    const actualizar = () => {
      const nuevos = JSON.parse(localStorage.getItem("productosCliente") || "[]");
      setProductos(nuevos);
    };

    window.addEventListener("productos-actualizados", actualizar);
    return () => window.removeEventListener("productos-actualizados", actualizar);
  }, []);
